...
if [ $? -eq 0 ]; then
	...
	export COMPILE="env CPPDEFINES=UPDATESERVER=\\\"starcatcher.us/TPT\\\" /c/Python27/python.exe /c/Python27/Scripts/scons.py --msvc --static --snapshot-id=$VER -j2"
	...
	if test $ret -ne 0; then
		...
		if true; then #grep -Fxq "cannot update program database" output_MSVC.log; then
		...
		echo "msg #powder-dev test2 please ignore" | ./nc.exe -w 1 localhost 9876
	fi
	popd
	exit $ret
fi
